...
def build_mmoe(features, params, scope, task_num=2):
  inputs, _ = build_input(features, params)
...
def grad_norm(loss_list, weights_shared):
  ...
  for i in range(task_num):
    ...
    g_norm = tf.norm(tf.concat(grads,axis=0), ord=2)
  ...
  C_list = [tf.stop_gradient(tf.identity(C_list[i])) for i in range(task_num)]
...
def esmm_model_fn(features, labels, mode, params):
  ...
  mmoe_scope = 'mmoe'
  inputs, shared_weights = build_mmoe(features, params, mmoe_scope)
  ...
  if params['model'] == 'dnn':
    with tf.variable_scope(dnn_parent_scope):
      with tf.variable_scope('dnn_ctr'):
        ctr_logits = build_deep_layers(inputs[0], hidden_units, mode, params['ctr_reg'])
      ...
      with tf.variable_scope('dnn_cvr'):
        cvr_logits = build_deep_layers(inputs[1], hidden_units, mode, params['cvr_reg'])
  ...
  if mode == tf.estimator.ModeKeys.PREDICT:
    ...
    def _train_op_fn(loss):
      ...
      if params['model'] in ('dnn'):
        ...
        dnn_var_list = ops.get_collection(ops.GraphKeys.TRAINABLE_VARIABLES, scope=dnn_parent_scope) + ops.get_collection(ops.GraphKeys.TRAINABLE_VARIABLES, scope='dnn_embed') + ops.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=mmoe_scope)
        train_ops.append(
